...
function getArtistByIndex(arr, index) {
  let artistsByIndex = artists[index]
  return `The artist at index ${artistsByIndex.id} is ${artistsByIndex.name}`
}
...
function get20s(arr) {
  ...
  for (i = 0; i < arr.length; i++){
    const birthday = Number(arr[i].years.slice(0, 4));
...
function addArtist(addArr) {
  let meAsAnArtist = ({id: 20, name: 'Me', years: '1990-Current', genre: 'Metalcore', nationality: 'Central European', bio: 'Just a guy learning Code. And having fun well doing it.'});
  return artists.push(meAsAnArtist);
}
